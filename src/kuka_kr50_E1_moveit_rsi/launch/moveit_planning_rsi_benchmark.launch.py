...
def launch_setup(context, *args, **kwargs):
    ...
    moveit_config = (
        ...
        .trajectory_execution(file_path="config/moveit_controller_E1.yaml")
    ...
    # --- Benchmark driver node ---
    driver_node = Node(
        package="kuka_kr50_E1_moveit_rsi",
        executable="kr50_E1_benchmark",
        name="kr50_E1_benchmark",          # optional: set explicit node name
        namespace=ns,                      # optional: respect your namespace arg
        output="screen",
        parameters=[
            moveit_config.to_dict(),

            # Hard-coded benchmark settings:
            {"benchmark_enabled": False},   # start disabled; enable via /benchmark_enable service
            {"benchmark_csv_path": "/mnt/nova_ssd/workspaces/isaac_ros-dev/kr50_benchmark_log.csv"},
        ],
    )
...
def generate_launch_description():
    launch_arguments = [
        DeclareLaunchArgument("robot_model", default_value="kr50_r2100"),
        DeclareLaunchArgument("robot_family_support", default_value="kuka_radars_support"),
